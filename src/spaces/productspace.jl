...
Return the number of index spaces in a product space.
"""
...
Return the axes for all index spaces in product space `P`.

    axes(P::ProductSpace, n::Int)

Return the axes for `n`th index space of product space `P`.

    axes(P::ProductSpace{<:ElementarySpace, N}, sectors::NTuple{N, <:Sector}) where {N}

Return the axes of `sectors[i]` in `P.spaces[i]` for all `i ∈ 1:N`.
"""
...
Return the product space with reversed order of index spaces of the input product space and
take the dual of each index space.
"""
