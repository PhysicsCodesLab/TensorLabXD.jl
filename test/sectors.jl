...
println("------------------------------------")
ti = time()
@timedtestset "Sector properties of $(TensorXD.type_repr(I))" for I in sectorlist
    Istr = TensorXD.type_repr(I)
    @testset "Sector $Istr: Basic properties" begin
        s = (randsector(I), randsector(I), randsector(I))
        @test eval(Meta.parse(sprint(show, I))) == I
        @test eval(Meta.parse(TensorXD.type_repr(I))) == I
    ...
    @testset "Sector $Istr: Value iterator" begin
        ...
        for (i, s) in enumerate(values(I))
            ...
            if Base.IteratorSize(values(I)) == Base.IsInfinite() && I <: ProductSector
                ...
                @test_throws ArgumentError TensorXD.findindex(values(I), s)
            elseif hasmethod(Base.getindex, Tuple{typeof(values(I)), Int})
                @test s == @constinferred (values(I)[i])
                @test TensorXD.findindex(values(I), s) == i
            end
            sprev = s
            i >= 10 && break
        end
        ...
        if Base.IteratorSize(values(I)) == Base.IsInfinite() && I <: ProductSector
            @test_throws ArgumentError TensorXD.findindex(values(I), one(I))
        elseif hasmethod(Base.getindex, Tuple{typeof(values(I)), Int})
            @test (@constinferred TensorXD.findindex(values(I), one(I))) == 1
            for s in smallset(I)
                @test (@constinferred values(I)[TensorXD.findindex(values(I), s)]) == s
            end
        end
    end
