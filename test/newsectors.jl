...
using HalfIntegers, WignerSymbols, TensorXD

struct NewSU2Irrep <: TensorXD.Sector
    j::HalfInt
...
TensorXD.:⊗(s1::NewSU2Irrep, s2::NewSU2Irrep) =
    TensorXD.SectorSet{NewSU2Irrep}(abs(s1.j-s2.j):(s1.j+s2.j))

Base.IteratorSize(::Type{TensorXD.SectorValues{NewSU2Irrep}}) = Base.IsInfinite()
Base.iterate(::TensorXD.SectorValues{NewSU2Irrep}, i = 0) = (NewSU2Irrep(half(i)), i+1)
...
# TensorXD.dim(s::NewSU2Irrep) = twice(s.j)+1
#
TensorXD.FusionStyle(::Type{NewSU2Irrep}) = GenericFusion()
TensorXD.BraidingStyle(::Type{NewSU2Irrep}) = Bosonic()
Base.isreal(::Type{NewSU2Irrep}) = true

TensorXD.Nsymbol(sa::NewSU2Irrep, sb::NewSU2Irrep, sc::NewSU2Irrep) =
    convert(Int, WignerSymbols.δ(sa.j, sb.j, sc.j))
function TensorXD.Fsymbol(s1::NewSU2Irrep, s2::NewSU2Irrep, s3::NewSU2Irrep,
...
function TensorXD.Rsymbol(sa::NewSU2Irrep, sb::NewSU2Irrep, sc::NewSU2Irrep)
...
function TensorXD.fusiontensor(a::NewSU2Irrep, b::NewSU2Irrep, c::NewSU2Irrep)
